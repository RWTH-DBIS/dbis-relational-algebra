...
class Union(ra.Operator):
    """
    This class represents a union in relational algebra
    """
    ...
    def __repr__(self) -> str:
        return f"({self.children[0]} \\cup {self.children[1]})"
    ...
    def evaluate(self, sql_con: Optional[sqlite3.Connection] = None) -> ra.Relation:
        ...
        for right_row in [tuple(row) for row in right_relation.rows]:
            new_relation.add_row(right_row)
        return new_relation
